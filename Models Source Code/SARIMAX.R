...
training_set <- paste0("SARIMAX_", dataset_name)
...
# Contains tailored configurations for different datasets:
#   - aggregation_minutes: temporal resolution options
#   - forecast_periods: prediction horizons in minutes
#   - validation_chunks: number of validation segments
#   - max_ar/ma: non-seasonal ARIMA parameters
#   - max_sar/sma: seasonal ARIMA parameters
#   - seasonal_period: dataset-specific seasonality frequency
...
validation_chunks <- current_params$validation_chunks
max_ar <- current_params$max_ar
max_ma <- current_params$max_ma
max_sar <- current_params$max_sar
max_sma <- current_params$max_sma
base_seasonal_period <- current_params$seasonal_period
...
# Implementation of SARIMAX model with comprehensive preprocessing
# Trains and evaluates SARIMAX models for specified forecast horizons
# Uses chunk-based validation approach for robust performance assessment
# Includes fallback model configurations for convergence failures
# Returns: comprehensive list of performance metrics and final model results

run_sarimax_preprocessed <- function(forecast_minutes, aggregationMinutes) {
  forecast_periods <- ceiling(forecast_minutes / aggregationMinutes)
  pm25Length <- length(tsPM25_preprocessed)
  chunk_size <- floor(pm25Length/5)
  chunk_points <- c(chunk_size, chunk_size*2, chunk_size*3, chunk_size*4)
  mae_train_values <- c()
  mae_test_values <- c()
  rmse_train_values <- c()
  rmse_test_values <- c()
  ...
  for (i in chunk_points) {
    if (i + forecast_periods > pm25Length) {
      next
    }
    ...
    tryCatch({
      tryCatch({
        model <- auto.arima(train_y, 
                            max.p = max_ar, max.q = max_ma, max.d = 1,
                            max.P = max_sar, max.Q = max_sma, max.D = 0,
                            seasonal = TRUE,
                            stepwise = TRUE, 
                            approximation = (length(train_y) > 150),
                            xreg = train_xreg,
                            seasonal.test = "ocsb",
                            ic = "aicc")
      }, error = function(e) {
        if(forecast_minutes <= 180) {
          model <- Arima(train_y, 
                         order = c(1,1,1), 
                         seasonal = list(order = c(1,0,1), period = seasonal_frequency),
                         xreg = train_xreg)
        } else {
          model <- Arima(train_y, 
                         order = c(2,1,2), 
      ...
      forecasted$upper <- pmax(0, forecasted$upper)
      ...
      if (min_length > 0) {
        ...
        safe_mape <- function(actual, predicted) {
          valid_idx <- which(actual > 1)
          if (length(valid_idx) == 0) return(NA)
          mean(abs((actual[valid_idx] - predicted[valid_idx]) / actual[valid_idx]) * 100, na.rm = TRUE)
        }
      ...
      cat("Error in preprocessed modeling chunk", i, "for", forecast_minutes, "minute forecast:", 
          as.character(e), "\n")
    })
  }
...
# Implementation of SARIMAX model with minimal preprocessing
# Uses basic imputation without advanced feature engineering
# Serves as baseline comparison to quantify preprocessing benefits
# Maintains identical evaluation framework to the preprocessed version
# Handles potential convergence issues with fallback configurations

run_sarimax_unprocessed <- function(forecast_minutes, aggregationMinutes) {
  forecast_periods <- ceiling(forecast_minutes / aggregationMinutes)
  pm25Length <- length(tsPM25_unprocessed)
  chunk_size <- floor(pm25Length/validation_chunks)
  chunk_points <- seq(chunk_size, by=chunk_size, length.out=validation_chunks-1)
  mae_train_values <- c()
  mae_test_values <- c()
  rmse_train_values <- c()
  rmse_test_values <- c()
    ...
    tryCatch({
      tryCatch({
        ...
        if(forecast_minutes <= 180) {
          model <- Arima(train_y, 
                         order = c(1,1,1), 
                         seasonal = list(order = c(1,0,1), period = seasonal_frequency_unprocessed),
                         xreg = train_xreg)
        } else {
          model <- Arima(train_y, 
                         order = c(2,1,2), 
      ...
      cat("Error in unprocessed modeling chunk", i, "for", forecast_minutes, "minute forecast:", 
          as.character(e), "\n")
    })
  }
  safe_mean <- function(x) {
...
for (aggregationMinutes in aggregation_minutes_list) {
  ...
  # Comprehensive preprocessing pipeline for SARIMAX:
  # 1. Missing value imputation using Kalman filtering
  # 2. Temporal feature engineering (hour, day, cyclical transformations)
  # 3. Advanced seasonal feature creation (day-of-week, weekend indicators)
  # 4. Multiple lag features based on dataset characteristics
  # 5. Feature standardization for improved model stability
  
  cat("\n===== Running SARIMAX WITH Preprocessing (Aggregation:", aggregationMinutes, "minutes) =====\n")
  ...
  lag_periods <- c(1, 2, 24*(60/aggregationMinutes))  
  lag_periods <- lag_periods[lag_periods < nrow(aggregatedData_preprocessed)/10]
  for (lag in lag_periods) {
    col_name <- paste0("pm25_lag", lag)
    aggregatedData_preprocessed[[col_name]] <- lag(aggregatedData_preprocessed$pm25, lag)
  }
  aggr_cols <- colnames(aggregatedData_preprocessed)
  lag_cols <- grep("^pm25_lag", aggr_cols, value = TRUE)
  for (col in lag_cols) {
    aggregatedData_preprocessed[[col]] <- ifelse(is.na(aggregatedData_preprocessed[[col]]), 
                                                 mean(aggregatedData_preprocessed[[col]], na.rm = TRUE), 
                                                 aggregatedData_preprocessed[[col]])
  }
  modelPredictors_preprocessed <- aggregatedData_preprocessed %>%
    dplyr::select(pm25, pm10, co2, temperature, humidity, voc, 
                  hour_sin, hour_cos, dow_sin, dow_cos, is_weekend, 
                  all_of(lag_cols)) %>%
    as.data.frame()
  modelPredictors_preprocessed[, -1] <- scale(modelPredictors_preprocessed[, -1])
  
  # Dynamic seasonality determination
  # Adapts seasonal period based on data availability
  # Ensures appropriate seasonality specification for shorter datasets
  # Prevents overparameterization when insufficient data is available
  
  observations_per_day <- 24 * (60/aggregationMinutes)
  if(nrow(aggregatedData_preprocessed) < observations_per_day * 3) {
    seasonal_frequency <- min(base_seasonal_period, floor(nrow(aggregatedData_preprocessed)/3))
  } else {
    seasonal_frequency <- base_seasonal_period
  }
  tsPM25_preprocessed <- ts(modelPredictors_preprocessed$pm25, frequency=seasonal_frequency)
  for(i in 1:nrow(results_preprocessed)) {
    cat("Processing forecast period WITH preprocessing:", results_preprocessed$Hours[i], "hours\n")
    model_results <- run_sarimax_preprocessed(results_preprocessed$Minutes[i], aggregationMinutes)
  ...
  cat("\n===== Running SARIMAX WITHOUT Preprocessing (Aggregation:", aggregationMinutes, "minutes) =====\n")
  ...
  modelPredictors_unprocessed <- aggregatedData_unprocessed %>%
    dplyr::select(pm25, pm10, co2, temperature, humidity, voc) %>%
    as.data.frame()
  seasonal_frequency_unprocessed <- 12  
  tsPM25_unprocessed <- ts(modelPredictors_unprocessed$pm25, frequency=seasonal_frequency_unprocessed)
  for(i in 1:nrow(results_unprocessed)) {
    ...
    model_results <- run_sarimax_unprocessed(results_unprocessed$Minutes[i], aggregationMinutes)
    ...
    final_results_unprocessed_list[[paste("min_", as.character(results_unprocessed$Minutes[i]))]] <- model_results$final_results
  }
  ...
  create_forecast_plot <- function(actual, forecast, lower_ci, upper_ci, time_index, aggregationMinutes, 
    ...
    p <- ggplot(plot_data, aes(x = Index)) +
      ...
      labs(
        title = paste0("SARIMAX ", ifelse(preprocessing_status == "Yes", "WITH", "WITHOUT"), 
  ...
  # Creates side-by-side comparisons between preprocessed and unprocessed models
  # Handles edge cases like missing results or insufficient data points
  # Generates both graphical comparisons and quantitative metric visualizations
  # Outputs standardized plots for inclusion in final report
  ...
  create_comparison_plots <- function() {
    ...
    for (hour in comparison_hours) {
      for (agg_min in aggregation_minutes_list) {
        ...
        unpre_results <- final_results_unprocessed_list[[paste("min_", as.character(forecast_min))]]
        ...
        compare_plot <- ggplot(compare_data, aes(x = Index)) +
          ...
          scale_fill_manual(name = "AQI Category", values = setNames(aqi_bands$color, aqi_bands$category)) +
          labs(
            title = paste0("SARIMAX Comparison: ", hour, "-Hour Forecast"),
        ...
        plot_filename <- paste0(plots_dir, "/SARIMAX_comparison_",
        ...
        tryCatch({
          ...
          expanded_metrics_plot <- ggplot(
            ...
            labs(
              title = paste0("SARIMAX Performance Metrics: ", hour, "-Hour Forecast"),
          ...
          expanded_metrics_filename <- paste0(plots_dir, "/SARIMAX_expanded_metrics_", 
          ...
          metrics_plot <- ggplot(metrics_long, aes(x = Metric, y = Value, fill = Model)) +
            ...
            labs(
              title = paste0("SARIMAX Test Performance: ", hour, "-Hour Forecast"),
          ...
          metrics_filename <- paste0(plots_dir, "/SARIMAX_metrics_test_", 
                                     agg_min, "min_", hour, "h_forecast", "_", 
  ...
  # Visualization framework for SARIMAX models
  # Creates forecast plots with confidence intervals
  # Incorporates AQI band overlays for health context interpretation
  # Color-coded to distinguish between actual values and model predictions
  # Provides standardized output format for result comparison
  ...
  for (aggregationMinutes in aggregation_minutes_list) {
    ...
    for (forecast_min in forecast_periods) {
      ...
      if (!is.null(pre_results) && !is.null(pre_results$actual) && length(pre_results$actual) > 0) {
        ...
        plot_filename <- paste0(plots_dir, "/SARIMAX_preprocessed_", 
      ...
      unpre_key <- paste("min_", as.character(forecast_min))
      ...
      if (!is.null(unpre_results) && !is.null(unpre_results$actual) && length(unpre_results$actual) > 0) {
        ...
        plot_filename <- paste0(plots_dir, "/SARIMAX_unprocessed_", 
                                aggregationMinutes, "min_", 
                                forecast_hrs, "h_forecast", "_", 
                                format(Sys.time(), "%Y%m%d_%H%M%S"),".png")
